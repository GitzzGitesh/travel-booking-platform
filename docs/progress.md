...
| 7 | **Next (proposed, technically unblocked): the exposure checklist** below (forwarded headers and per-client rate limiting on the four anonymous flight endpoints, with the endpoint-metadata test). Phase 3 (Order aggregate, authorize → book → capture, persisted items with `PendingConfirmation` and Worker reconciliation using `FlightSupplierBooking`) needs approval and Q1 (merchant of record) | Proposed |
