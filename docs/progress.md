...
| 6 | **Next (proposed): the remaining flight port operations without orders or payment.** `BookAsync` with our client reference and `RetrieveAsync` by client reference (mandatory for reconciliation), with the mock scenarios (`rejected`, `timeout-then-booked`, `timeout-not-booked`) and the provider contract suite. The Order aggregate and authorize → book → capture belong to Phase 3, which needs approval and Q1 (merchant of record) | Proposed |
...
- **Exposure checklist: before flight search or offer selection is mapped outside Development** (security and architecture reviews of chunks 2 and 4a):
  - Forwarded headers, then per-client rate limiting via `.RequireRateLimiting(...)` in `MapFlightsEndpoints` on all four endpoints (search, selection, revalidation, price acceptance; revalidation calls the supplier), plus an endpoint-metadata test that every anonymous `/api/v1` endpoint is rate limited.
