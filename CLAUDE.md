...
- Run the Worker (background jobs: outbox, payment reconciliation, order expiry): `dotnet run --project src/backend/Hosts/Worker`, with the same connection strings as the Api
...
- Database (SQL Server, one schema per module): the Orders module owns schema `orders` (connection string `Orders`; migrations as for Flights with `--project`/`--startup-project src/backend/Modules/Orders/Modules.Orders`). The Payments module owns schema `payments` (connection string `Payments`; likewise with `src/backend/Modules/Payments/Modules.Payments`). The Flights module owns schema `flights` (connection string `Flights`, set with user-secrets locally). Migrations use the local tool (`dotnet tool restore`): `dotnet ef migrations add <Name> --project src/backend/Modules/Flights/Modules.Flights --startup-project src/backend/Modules/Flights/Modules.Flights --output-dir Infrastructure/Migrations`, then review `dotnet ef migrations script`. Migrations are never applied at startup.
- Database-backed tests (`Modules.Flights.IntegrationTests`, and the `SqlApiFactory` classes in `Api.IntegrationTests`: selection, revalidation, supplier booking, order creation, payments and background processing) use Testcontainers and need Docker. If Docker is only reachable inside WSL, run them there with a separate output path: `dotnet test --project tests/backend/<Project> --artifacts-path /tmp/tb-artifacts`.
